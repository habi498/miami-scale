...
SQInteger fn_MiamiScale_Init(HSQUIRRELVM v)
{
	...
	*/
	...
	Server->LogMessage("MiamiScale Init began.");
	...
	delete[] part2;
	init = true;
	Server->LogMessage("MiamiScale Init completed.");
...
uint8_t MiamiScale_OnPluginCommand(uint32_t commandIdentifier, const char* message)
{
...
extern "C"  unsigned int VcmpPluginInit(PluginFuncs * pluginFuncs, PluginCallbacks * pluginCallbacks, PluginInfo * pluginInfo) {
	...
	memcpy(pluginInfo->name, "MiamiScale", 10); pluginInfo->name[10] = 0;
	
	pluginCallbacks->OnServerInitialise = MiamiScale_OnServerInitialize;

	pluginCallbacks->OnPluginCommand = MiamiScale_OnPluginCommand;
	return 1;
}
